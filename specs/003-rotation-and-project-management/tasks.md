...
- [ ] P003F006T001 Verify `#viewport-container` in `src/style.css` has `position: relative` (required for the absolutely-positioned `#gizmo-toolbar` to anchor within the viewport); add `position: relative` to the `#viewport-container` CSS rule if absent
- [ ] P003F006T002 Wire `createGizmoToolbar` in `src/main.ts`: import from `./scene/GizmoToolbar.js`; after the `viewportContainer` guard, call `createGizmoToolbar(viewportContainer, viewport.getTransformGizmo())`; run `pnpm typecheck && pnpm test` — must pass
